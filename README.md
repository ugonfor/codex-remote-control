...
host agent가 pairing code, pairing URL, QR을 출력합니다. 핸드폰에서 URL을 열거나 QR을
스캔하면 연결됩니다.
...
tmux new-session -d -s codex-remote-relay \
  'cd /path/to/codex-remote-control && PORT=8787 PUBLIC_RELAY_URL=http://172.30.1.24:8787 MOBILE_APP_URL=http://172.30.1.24:5173 PAIRING_TTL_MS=86400000 npm run dev:relay'
```
...
1. 핸드폰에서 `http://<mac-ip>:5173` 또는 pairing URL을 엽니다.
2. pairing URL로 열면 자동으로 pairing을 시도합니다.
3. 연결 후 왼쪽 Recents에서 online host를 선택합니다.
